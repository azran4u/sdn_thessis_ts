...
export class GraphUtil {
    ...
    static findHigherLayerRequests(requests: VideoRequest[], request: VideoRequest): VideoRequest[] {
    ...
    static removeEdgesWithoutEnoughBw(G: graphlib.Graph, minBw: number): graphlib.Graph {
    ...
    static updatePathBwInGraph(path: NetworkPath, bw: number, G: graphlib.Graph): graphlib.Graph {
        path.edges.forEach(edge => {
            // update G with the served bandwidth
            const edgeBefore = G.edge(edge.from_node, edge.to_node) as NetworkEdge;
            G.setEdge(edge.from_node, edge.to_node, { ...edgeBefore, bw: (edgeBefore.bw - bw) });
        })
        return G;
    }

    static addPathToTree(pathToGraph: PathToTree, G: graphlib.Graph): graphlib.Graph {
        let currentNode = pathToGraph.node;
        const edges = pathToGraph.path.edges;
        while (edges.length > 0) {
            const currentEdgeIndex = edges.findIndex(edge => { return edge.from_node === currentNode.id });
            if (currentEdgeIndex === -1) { // not found
